...
class MindQGuardianAdapter:
    ...
    def _extract_kpis(self, guide_content: str) -> List[Dict[str, str]]:
        """Extract KPIs mentioned in the guide."""
        kpis = [
            {"name": "RTO%", "description": "Return to Origin percentage", "category": "delivery"},
            {"name": "SLA", "description": "Service Level Agreement compliance", "category": "service"},
            {"name": "COD", "description": "Cash on Delivery collection rate", "category": "financial"},
            {"name": "PSI", "description": "Population Stability Index for data drift", "category": "quality"},
            {"name": "NZV", "description": "Near Zero Variance for feature stability", "category": "quality"},
            {"name": "Delivery Success", "description": "Successful delivery rate", "category": "delivery"},
            {"name": "Hub Efficiency", "description": "Hub throughput and processing time", "category": "operations"}
        ]
        return kpis
    ...
    def _extract_phase_section_from_guide(self, phase_id: str) -> Dict[str, Any]:
        """Extract phase-specific section from guide."""
        if not self.guide_path.exists():
            return {}
        ...
        section_data = {
            "goal": f"Process data through {self._phase_id_to_name(phase_id)}",
            "inputs": ["Previous phase outputs"],
            "outputs": ["Processed artifacts"],
            "kpis": [],
            "business_impact": "Improves data quality and pipeline reliability"
        }
        
        # Extract business objective if present
        business_obj_match = re.search(
            rf"### Stage {phase_id.split('_')[0]}.*?#### Business Objective\s+(.*?)(?=####|\n###|\Z)",
            content,
            re.DOTALL
        )
        if business_obj_match:
            section_data["business_impact"] = business_obj_match.group(1).strip()[:200]
...
"""
    ...
    def get_guardian_context_for_request(self, user_request: str) -> str:
        ...
        # Provide detailed context for relevant phases
        return self.get_phase_context(relevant_phases[:3])  # Limit to 3 phases
